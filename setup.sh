...
function safe_link () {
    ...
    if [ -e "$link_name" ]; then
        if [ -L "$link_name" ]; then
            echo "${RED}Cannot create link from $target to $link_name. $link_name is already a link${NC}"
        else
            echo "${RED}Cannot create link from $target to $link_name. $link_name exists but is not a link${NC}"
        fi
    else
...
brew install google-chrome visual-studio-code emacs neovim starship htop rectangle
